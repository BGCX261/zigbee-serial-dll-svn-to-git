...
void CDLLTestDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT3, m_devid_1);
	DDX_Text(pDX, IDC_EDIT5, m_devid_2);
...
BOOL CDLLTestDlg::OnInitDialog()
{
	...
	m_strColumnbar = "  시간\t\t제품ID\tCommand \tData";
	UpdateData(false);
...
void CDLLTestDlg::OnBnClickedButton1()
{
	...
	BYTE comm[2] = {StringToByte(m_command_1),StringToByte(m_command_2)};
	BYTE data[30];
	for(int i=0; i<30; i++)
	{
		data[i] = 0x00;
...
void CDLLTestDlg::OnBnClickedButton2()
{
	...
	id[0] = StringToByte(m_devid_1);
	id[1] = StringToByte(m_devid_2);
	...
	if(zbDataRead(StringToByte(m_command_2),id)){		
		//strLogMsg.Format("[SEND TIME : %02d-%02d %02d:%02d:%02d]\r\n",ct.GetMonth(),ct.GetDay(),ct.GetHour(),ct.GetMinute(),ct.GetSecond());
...
void CDLLTestDlg::OnBnClickedButton4()
{
	...
	if(zbCommOver(StringToByte(m_command_2),id)){
		//strLogMsg.Format("[SEND TIME : %02d-%02d %02d:%02d:%02d]\r\n",ct.GetMonth(),ct.GetDay(),ct.GetHour(),ct.GetMinute(),ct.GetSecond());
...
LRESULT CDLLTestDlg::receivePacket(WPARAM wParam, LPARAM lParam)
{
	...
	if(dataType == GENERAL_DATA){
		strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],0x10);
		strTmp = "";
		for(int i=0; i<sizeof(rcvData.SECTION);i++){
			tmpChar.Format(" %02x",rcvData.SECTION[i]);
			strTmp += tmpChar;
		};
		strLogMsg += "\t\t"+strTmp;
		...
		strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],0x20);
		strTmp = "";
		for(int i=0; i<sizeof(rcvData.FACILITY);i++){
			tmpChar.Format(" %02x",rcvData.FACILITY[i]);
			strTmp += tmpChar;
		};
		strLogMsg += "\t\t"+strTmp;
		...
		strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],0x30);
		strTmp = "";
		for(int i=0; i<sizeof(rcvData.PROVIDER);i++){
			tmpChar.Format(" %02x",rcvData.PROVIDER[i]);
			strTmp += tmpChar;
		};
		strLogMsg += "\t\t"+strTmp;
		...
		strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],0x40);
		strTmp = "";
		for(int i=0; i<sizeof(rcvData.CUSTOMER);i++){
			tmpChar.Format(" %02x",rcvData.CUSTOMER[i]);
			strTmp += tmpChar;
		};
		strLogMsg += "\t\t"+strTmp;
		...
		strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x\t\tTEMPERATURE = %.1f HUMIDITY = %.1f%% MOISTURE = %d",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],50,rcvData.TEMPERATURE,rcvData.HUMIDITY,rcvData.MOISTURE);
		...
		if(bResult){
			strLogMsg = "";
			strLogMsg.Format("  %02d:%02d:%02d\t%02x %02x\t     %02x",ct.GetHour(),ct.GetMinute(),ct.GetSecond(),rcvData.ID[0],rcvData.ID[1],0x00);
			strTmp = "";
